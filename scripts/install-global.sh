#!/usr/bin/env bash
# install-global.sh — install the waypoint CLI and enable it for EVERY Claude
# Code project (global). Task state always stays per-project in each project's
# .claude/waypoint/; only the tooling (CLI + hooks + skill) is global.
#
...
python3 - "$DEST/settings.json" "$DEST/hooks" <<'PY'
...
echo "[waypoint] done — $(waypoint --version 2>/dev/null || echo 'CLI installed')."
